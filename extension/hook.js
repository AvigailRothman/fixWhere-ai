...
(function() {
  const send = (payload) => {
    // שלב קריטי: הפיכה ל-JSON. 
    // אם ה-stack כבר קיים ב-payload, נשתמש בו.
    window.dispatchEvent(new CustomEvent("DEBUG_CATCHER_EVENT", {
      detail: JSON.parse(JSON.stringify(payload, (key, value) => 
        value instanceof Error ? { message: value.message, stack: value.stack } : value
      ))
  ...
  window.addEventListener("error", (e) => {
    // אנחנו יוצרים את ה-fallback לסטאק בדיוק ברגע השגיאה
    const stackTrace = e.error?.stack || new Error().stack;
    send({ 
        source: "window.error", 
        kind: "runtime-error", 
        message: e.message, 
  ...
  window.addEventListener("unhandledrejection", (e) => {
    // יצירת סטאק fallback מידית אם ה-reason לא מכיל אחד
    const stackTrace = e.reason?.stack || new Error().stack;
    send({ 
  ...
  window.__REPORT_ERROR__ = (p) => {
    // אם ריאקט שלח שגיאה בלי סטאק, ניצור אחד
    const payload = { ...p, source: "react.error-boundary" };
  ...
  console.error = (...args) => {
    ...
    const msg = args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' ');
    if (!msg.includes("Vite")) {
       // כאן זה כבר היה נכון - יוצר סטאק ברגע הקריאה ל-error
       send({ source: "console", kind: "error", message: msg, stack: new Error().stack });
    }
  };
})();